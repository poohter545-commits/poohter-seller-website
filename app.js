...
const downloadPdfWithQr = async (filename, title, lines, qrImageUrl) => {
  ...
  const rowCommands = detailLines.slice(0, 9).flatMap((line, index) => {
    const y = 622 - (index * 34);
    return [
      "q",
      index % 2 === 0 ? "0.97 0.99 1 rg" : "1 1 1 rg",
      `42 ${y - 8} 300 28 re f`,
      "Q",
      "BT",
      "/F2 10 Tf",
  ...
  const contentLines = [
    ...
    "/F2 10 Tf",
    "0 -19 Td",
    "(Attach this receipt with the physical product stock before warehouse receiving.) Tj",
    "ET",
    ...rowCommands,
    "q",
    "1 1 1 rg",
    "371 532 176 176 re f",
    "Q",
    "q",
    "0.88 0.94 0.95 RG",
    "371 532 176 176 re S",
    "Q",
    "q",
    "150 0 0 150 384 545 cm",
    "/Im1 Do",
    "Q",
    "BT",
    "0.05 0.30 0.38 rg",
    ...
    "/F1 12 Tf",
    "58 158 Td",
    "(Warehouse instruction) Tj",
    "/F2 10 Tf",
    "0 -18 Td",
    "(Scan this QR first. Scanner only confirms warehouse receiving; admin completes details before Top Team pricing.) Tj",
    "ET",
  ];
  const stream = contentLines.join("\n");
  const objects = [
